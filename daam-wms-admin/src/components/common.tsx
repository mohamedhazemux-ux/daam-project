...
export function Modal({ open, onClose, title, children, footer, wide }: {
  ...
  return (
    ...
    >
      <div className={cn('mx-auto my-8 w-full rounded-2xl border bg-card shadow-2xl animate-fade-up', wide ? 'max-w-3xl' : 'max-w-md')}>
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h3 className="text-[15px] font-extrabold">{title}</h3>
          <button className="inline-flex size-8 items-center justify-center rounded-md border border-input hover:bg-accent" onClick={onClose} aria-label="إغلاق"><X className="size-4" /></button>
        </div>
  ...
  return (
    <Modal
      ...
      footer={<>
        ...
        <Button variant={destructive ? 'destructive' : 'default'} disabled={loading} onClick={onConfirm}>{loading ? 'جارٍ التنفيذ...' : confirmLabel}</Button>
      </>}
    >
...
export function DonutChart({ data }: { data: { label: string; value: number; color: string }[] }) {
  ...
  const arcs = data.map(d => {
    ...
    acc += d.value
    const a2 = (acc / total) * 360
    const [sx, sy] = P(a1)
    const [ex, ey] = P(a2 - 0.8)
    return <path key={d.label} d={'M' + sx + ' ' + sy + ' A38 38 0 ' + (a2 - a1 > 180 ? 1 : 0) + ' 1 ' + ex + ' ' + ey} fill="none" stroke={d.color} strokeWidth="13" />
  })
  return (
