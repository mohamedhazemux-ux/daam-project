...
const seed = (): MerchantReturn[] => {
  ...
  return [{ ref: 'RET-' + letter + '-001', m: o.m, order: o.ref, cust: o.cust, phone: '0512345678', deliveredAt: o.date, items: [{ name: o.items[0]?.name ?? 'منتج', ref: 'PRD-001', qty: 1, reason: 'منتج تالف', condition: 'تالف', images: ['damage-1.jpg'] }], totalItems: 1, type: 'إتلاف', refundMethod: 'رصيد المحفظة', notes: '', attachments: ['damage-1.jpg'], status: 'في الطريق', createdAt: o.date, timeline: ['معلق — ' + o.date, 'معتمد بواسطة منى المطيري — ' + o.date, 'في الطريق — رقم التتبع TRK-778899 — ' + o.date] }]
}
