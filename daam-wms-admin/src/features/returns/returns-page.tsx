...
export default function ReturnsPage() {
  ...
  const approve = useMutation({ mutationFn: (ref: string) => returnsService.approve(ref), onSuccess: () => { toast.success(t('تم اعتماد طلب الإرجاع وتوليد بوليصة الشحن بنجاح')); invalidate(); setApproving(null) } })
  const reject = useMutation({ mutationFn: (v: { ref: string; reason: string }) => returnsService.reject(v.ref, v.reason), onSuccess: () => { toast.success(t('تم رفض طلب الإرجاع بنجاح')); invalidate(); setRejecting(null) } })
  const receive = useMutation({ mutationFn: (ref: string) => returnsService.receive(ref), onSuccess: () => { toast.success(t('تم تأكيد استلام القطع المرتجعة في المستودع بنجاح')); invalidate(); setReceiving(null) } })
  const inspect = useMutation({ mutationFn: (v: { ref: string; results: { condition: string }[] }) => returnsService.inspect(v.ref, v.results), onSuccess: () => { toast.success(t('تم تسجيل نتائج الفحص بنجاح')); invalidate(); setInspecting(null) } })
  const refund = useMutation({ mutationFn: (v: { ref: string; method: string; amount: number }) => returnsService.refund(v.ref, v.method, v.amount), onSuccess: () => { toast.success(t('تمت معالجة الاسترداد المالي بنجاح')); invalidate(); setRefunding(null) } })
  ...
  const columns: ColumnDef<ReturnRequest, unknown>[] = [
    ...
    { id: 'actions', header: t('إجراءات'), cell: ({ row }) => {
      ...
      <ActionButtons actions={[
        { icon: Eye, label: t('عرض التفاصيل'), onClick: () => navigate(`/records/return/${r.ref}`) },
        { icon: CheckCircle, label: t('اعتماد الطلب'), onClick: () => setApproving(r.ref), hidden: r.status !== 'معلق' },
        { icon: XCircle, label: t('رفض الطلب'), variant: 'destructive', onClick: () => { setRejecting(r.ref); setReason(''); setRErr('') }, hidden: r.status !== 'معلق' },
        { icon: Truck, label: t('استلام القطع'), onClick: () => setReceiving(r), hidden: r.status !== 'في الطريق' },
        { icon: ClipboardCheck, label: t('فحص القطع'), onClick: () => openInspect(r), hidden: r.status !== 'مستلم' },
        { icon: Wallet, label: t('معالجة الاسترداد'), onClick: () => { setRefunding(r); setMethod('رصيد المحفظة'); setNotes(''); setFErr('') }, hidden: r.status !== 'تم الفحص' },
      ]} />) } },
  ]
  ...
  return (
    <div className="rounded-xl border bg-card shadow-sm">
      ...
      <DataTable columns={columns} data={data?.rows ?? []} total={data?.total ?? 0} page={page} pageSize={10} onPageChange={setPage} loading={isLoading} getRowId={(r: ReturnRequest) => r.ref}
        toolbar={
          <div className="flex flex-wrap items-center gap-2 border-b p-3">
            ...
            <Button variant="outline" size="sm" className="ms-auto" onClick={() => { downloadCSV('returns', ['المرجع', 'التاجر', 'البريد', 'الطلب الأصلي', 'العميل', 'القطع', 'النوع', 'الحالة', 'التاريخ'], (data?.rows ?? []).map(r => [r.ref, r.m, r.email, r.order, r.cust, r.count, r.type, r.status, r.date])); toast.success(t('تم تصدير الملف بنجاح')) }}>{t('تصدير')}</Button>
          </div>
        } />

      <ConfirmDialog open={!!approving} onOpenChange={v => { if (!v) setApproving(null) }} title={t('اعتماد طلب الإرجاع')} loading={approve.isPending}
        description={t('هل أنت متأكد من اعتماد طلب الإرجاع') + ' ' + (approving ?? '') + '؟ ' + t('سيتم توليد بوليصة شحن الإرجاع وإرسال التعليمات للتاجر والعميل.')}
        confirmLabel={t('اعتماد')} onConfirm={() => approve.mutate(approving!)} />

      <Modal open={!!rejecting} onClose={() => setRejecting(null)} title={t('رفض طلب الإرجاع') + ' — ' + (rejecting ?? '')}
        footer={<>
      ...
      <ConfirmDialog open={!!receiving} onOpenChange={v => { if (!v) setReceiving(null) }} title={t('استلام القطع المرتجعة')} loading={receive.isPending}
        description={receiving ? <>{t('التاجر')}: <b>{receiving.m}</b> — {t('الطلب الأصلي')}: <b>{receiving.order}</b><br />{t('القطع المتوقعة')}: <b>{receiving.count}</b> — {t('تأكيد الاستلام الفعلي وتحديث الحالة إلى')} "{t('مستلم')}"؟</> : undefined}
        confirmLabel={t('تأكيد الاستلام')} onConfirm={() => receive.mutate(receiving!.ref)} />

      <Modal open={!!inspecting} onClose={() => setInspecting(null)} wide title={t('فحص القطع المرتجعة') + ' — ' + (inspecting?.ref ?? '')}
        footer={<>
          <Button variant="outline" onClick={() => setInspecting(null)}>{t('إلغاء')}</Button>
          <Button disabled={inspect.isPending} onClick={() => {
            if (conds.some(c => !c.condition)) { setIErr(t('الحالة الفعلية مطلوبة لكل القطع')); return }
            if (conds.some(c => c.notes.length > 300)) { setIErr(t('ملاحظات الفاحص يجب أن تكون أقل من 300 حرف')); return }
            setIErr('')
            inspect.mutate({ ref: inspecting!.ref, results: conds })
          }}>{t('إكمال الفحص')}</Button>
        </>}>
        <div className="space-y-3">
          {conds.map((c, i) => (
            <div key={i} className="rounded-lg border bg-muted/40 p-3">
              <p className="mb-2 text-[13px] font-extrabold">{t(ITEM_NAMES[i % ITEM_NAMES.length])} — {t('كمية')} 1</p>
              <div className="grid gap-3 md:grid-cols-2">
                <div><Label>{t('الحالة الفعلية')} <span className="text-destructive">*</span></Label>
                  <select className={selectCls + ' w-full'} value={c.condition} onChange={e => setConds(s => s.map((x, j) => j === i ? { ...x, condition: e.target.value } : x))}>
                    <option value="">{t('اختر الحالة...')}</option>
                    {CONDITIONS.map(x => <option key={x} value={x}>{t(x)}</option>)}
                  </select></div>
                <div><Label>{t('ملاحظات الفاحص (اختياري — 300 حرف)')}</Label>
      ...
      <Modal open={!!refunding} onClose={() => setRefunding(null)} title={t('معالجة الاسترداد') + ' — ' + (refunding?.ref ?? '')}
        footer={<>
          <Button variant="outline" onClick={() => setRefunding(null)}>{t('إلغاء')}</Button>
          <Button disabled={refund.isPending} onClick={() => {
            if (notes.length > 300) { setFErr(t('ملاحظات الاسترداد يجب أن تكون أقل من 300 حرف')); return }
            setFErr('')
            refund.mutate({ ref: refunding!.ref, method, amount: refundAmount(refunding!) })
          }}>{t('تأكيد الاسترداد')}</Button>
        </>}>
        {refunding && <>
          <div className="mb-3 grid grid-cols-2 gap-3">
            {[[t('إجمالي الطلب الأصلي'), money(refundAmount(refunding) + 25)], [t('تكلفة الشحن (غير مستردة)'), money(25)], [t('الخصومات (قطع تالفة)'), money(0)], [t('صافي المبلغ المسترد'), money(refundAmount(refunding))]].map(([k, v]) => (
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div><Label>{t('الملاحظات')} ({t('اختياري — 300 حرف')})</Label><Input value={notes} maxLength={300} onChange={e => setNotes(e.target.value)} /></div>
          </div>
