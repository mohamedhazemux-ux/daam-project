  ...
  const columns: ColumnDef<AppNotification, unknown>[] = [
    ...
    { id: 'actions', header: 'إجراءات', cell: ({ row }) => (
      <ActionButtons actions={[
        { icon: CheckCheck, label: 'تحديد كمقروء', onClick: () => { systemService.markRead(row.original.id); invalidate() }, hidden: !row.original.unread },
        { icon: Trash2, label: 'حذف الإشعار', variant: 'destructive', onClick: () => { systemService.removeNotification(row.original.id); invalidate(); toast.success('تم حذف الإشعار') } },
      ]} />) },
  ]
  return (
