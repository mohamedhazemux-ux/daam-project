...
import { Layers, PackagePlus, PackageMinus, Search } from 'lucide-react'
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 rounded-xl border bg-card p-2 shadow-sm">
        <button onClick={() => setTab('levels')} className={tab === 'levels' ? 'rounded-lg bg-foreground px-4 py-2 text-[13px] font-extrabold text-background' : 'rounded-lg px-4 py-2 text-[13px] font-bold text-muted-foreground hover:bg-accent'}>مستويات المخزون</button>
        <button onClick={() => setTab('requests')} className={tab === 'requests' ? 'rounded-lg bg-foreground px-4 py-2 text-[13px] font-extrabold text-background' : 'rounded-lg px-4 py-2 text-[13px] font-bold text-muted-foreground hover:bg-accent'}>طلبات المخزون</button>
      </div>
...
function LevelsSection() {
  ...
  return (
    <div className="space-y-4">
        ...
        <DataTable columns={columns} data={data?.rows ?? []} total={data?.total ?? 0} page={page} pageSize={10} onPageChange={setPage} loading={isLoading} getRowId={r => r.sku}
          toolbar={
            <div className="flex flex-wrap items-center gap-2 border-b p-3">
              ...
              <select className={selectCls} value={op} onChange={e => { setOp(e.target.value); setPage(1) }} aria-label="تصفية حسب الكمية المتاحة">
                <option value="">الكمية المتاحة: الكل</option>
      ...
      <Modal open={!!modal} onClose={() => setModal('')} title={modal === 'إضافة' ? 'طلب إضافة مخزون' : 'طلب سحب مخزون'}
        footer={<><Button variant="outline" onClick={() => setModal('')}>إلغاء</Button><Button disabled={submit.isPending} onClick={doSubmit}>إرسال الطلب</Button></>}>
...
function RequestsSection() {
  ...
  const columns: ColumnDef<StockRequest, unknown>[] = [
    ...
    { accessorKey: 'p', header: 'اسم المنتج' },
    { accessorKey: 'date', header: 'تاريخ الطلب' },
    { accessorKey: 'qty', header: 'الكمية' },
    { id: 'type', header: 'نوع الطلب', cell: ({ row }) => <span className="rounded-md bg-blue-50 px-2 py-0.5 text-[11px] font-extrabold text-blue-700">{row.original.type}</span> },
    { id: 'status', header: 'حالة الطلب', cell: ({ row }) => <StatusBadge value={statusLabel(row.original.status)} /> },
    { id: 'actions', header: 'إجراءات', cell: ({ row }) => <Button size="sm" variant="outline" onClick={() => setViewing(row.original)}>عرض التفاصيل</Button> },
  ]
  return (
    <div className="rounded-xl border bg-card shadow-sm">
      <DataTable columns={columns} data={data?.rows ?? []} total={data?.total ?? 0} page={page} pageSize={10} onPageChange={setPage} loading={isLoading} getRowId={r => r.id}
        toolbar={
          <div className="flex flex-wrap items-center gap-2 border-b p-3">
            <div className="relative min-w-[200px] flex-1 md:max-w-xs">
              <Search className="absolute end-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden />
              <Input value={q} onChange={e => { setQ(e.target.value); setPage(1) }} placeholder="بحث برقم الطلب أو المنتج..." className="pe-9" aria-label="بحث في طلبات المخزون" />
            </div>
            <select className={selectCls} value={type} onChange={e => { setType(e.target.value); setPage(1) }} aria-label="تصفية حسب النوع">
              <option value="">كل الأنواع</option><option value="إضافة">إضافة</option><option value="سحب">سحب</option>
            </select>
            <select className={selectCls} value={status} onChange={e => { setStatus(e.target.value); setPage(1) }} aria-label="تصفية حسب الحالة">
              <option value="">كل الحالات</option><option value="معلق">معلق</option><option value="معتمد">موافق عليه</option><option value="مرفوض">مرفوض</option>
            </select>
            <Button variant="outline" size="sm" className="ms-auto" onClick={() => { downloadCSV('stock-requests', ['الرقم', 'المنتج', 'التاريخ', 'الكمية', 'النوع', 'الحالة'], (data?.rows ?? []).map(r => [r.id, r.p, r.date, r.qty, r.type, statusLabel(r.status)])); toast.success('تم تصدير طلبات المخزون بنجاح') }}>تصدير</Button>
          </div>
        } />
