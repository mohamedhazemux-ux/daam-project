...
export interface Merchant { id: string; store: string; first: string; last: string; email: string; phone: string; status: EntityStatus; join: JoinStatus; created: string; gender: 'ذكر' | 'أنثى'; bank: string; iban: string; natId: string; natAddr: string; limit: number; used: number; unit: string; notes?: string }
...
export interface StockRequest { id: string; m: string; date: string; type: 'إضافة' | 'سحب'; qty: number; p: string; wh: string; status: 'معلق' | 'معتمد' | 'مرفوض' }
export interface StockLevel { p: string; sku: string; wh: string; avail: number; res: number }
export interface Order { id: string; m: string; date: string; status: 'معلق' | 'قيد المعالجة' | 'مكتمل' | 'ملغي'; items: number; total: number; ship: 'منصة' | 'ذاتي'; cust: string }
export interface ReturnRequest { ref: string; m: string; email: string; order: string; cust: string; count: number; type: string; date: string; status: string; reason?: string }
export interface Withdrawal { id: string; m: string; email: string; amount: number; method: string; bank: string; date: string; status: string }
...
export interface ServiceRequest { ref: string; m: string; email: string; type: string; prod: string; qty: number; cost: number; urgency: Urgency; date: string; req: string; status: string }
...
export interface Approval { id: string; type: string; urgency: string; who: string; title: string; date: string; days: number; qty?: number }
